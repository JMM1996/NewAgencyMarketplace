...
export async function DELETE(
  ...
  try {
    ...
    if (!employment || employment.careStaffId !== careStaff.id) {
...
export async function PATCH(
  ...
  try {
    ...
    // Verify ownership
    const existingEmployment = await prisma.employmentHistory.findUnique({
      where: { id },
    })

    if (!existingEmployment || existingEmployment.careStaffId !== careStaff.id) {
      return NextResponse.json({ error: 'Not found' }, { status: 404 })
    }
    ...
    const employment = await prisma.employmentHistory.update({
      where: { id },
      data: {
